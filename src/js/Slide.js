...
export default class Slide {
	...
	setClip() {
		gsap.set(this.DOM.cards.center.parentElement, {
			clipPath: calculateClipPath(this.isFullScreen ? this.DOM.cardsWrapperEl : this.DOM.cards.center).clipPath,
		});
	}
		...
		const onCardClickEv = () => {
				...
				.timeline({
					...
					onStart: () => {
						...
						if (clicked) {
							...
							if (this.offFullscreenCallbackFns) this.offFullscreenCallbackFns.forEach((fn) => fn());
						}
					},
					onComplete: () => {
						this.isAnimating = false;
			...
			tl.to(
				...
				.to(
					...
					{ clipPath: calculateClipPath(clicked ? this.DOM.cardsWrapperEl : this.DOM.cards.center).clipPath },
					clicked ? "upcoming" : "start"
				);
		};
