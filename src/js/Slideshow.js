...
export default class Slideshow {
  constructor(el) {
    ...
    this.slides = Array.from(this.DOM.el.querySelectorAll(".slide")).map(
      (slide) => new Slide(slide),
    );
    this.slideInfos = Array.from(this.DOM.el.querySelectorAll(".slider__slide-info")).map(
      (slideInfo) => new Slideinfo(slideInfo),
    );
  ...
  init() {
    ...
    this.slides.forEach((slide, index) => {
      if (currentSlide != slide) {
        gsap.set(
          [
            slide.DOM.cards.left.parentElement,
            slide.DOM.cards.center,
            slide.DOM.cards.right.parentElement,
          ],
          {
            x: "100vw",
          },
    ...
    gsap
      ...
      .set([currentSlide.DOM.cards.left, currentSlide.DOM.cards.right], {
        translateX: 0,
        rotate: 0,
        opacity: 0,
      })
      .set(currentSlide.DOM.cards.center, {
        translateY: -currentSlide.DOM.cards.center.clientHeight * 1.15,
      })
      .set(currentSlide.DOM.cards.center.children[0], {
        translateY: currentSlide.DOM.cards.center.clientHeight * 1.15,
      })
  ...
  initAnimation() {
    ...
    tl.to(currentSlide.DOM.el, { yPercent: 0 }, "start")
      ...
      .to(
        [currentSlide.DOM.cards.center, currentSlide.DOM.cards.center.children[0]],
        { translateY: 0 },
        "start",
      )
  ...
  navigate(direction) {
    ...
    const slideTl = gsap
      .timeline({
        ...
        onStart: () => {
          ...
          if (this.onSlideChangeCallbackFns)
            this.onSlideChangeCallbackFns.forEach((fn) => fn(this.current));
        },
        onComplete: () => {
          this.isAnimatingSlide = false;
    ...
    slideTl
      .to(
        [
          currentSlide.DOM.cards.left.parentElement,
          currentSlide.DOM.cards.center,
          currentSlide.DOM.cards.right.parentElement,
        ],
        {
      ...
      .fromTo(
        [
          nextSlide.DOM.cards.left.parentElement,
          nextSlide.DOM.cards.center,
          nextSlide.DOM.cards.right.parentElement,
        ],
        { x: direction === "next" ? "100vw" : "-100vw" },
        {
          x: 0,
